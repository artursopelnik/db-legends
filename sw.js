...
self.addEventListener('fetch', (event) => {
  ...
  // Blog, Team-Guide + Promo-Codes-Seite (alle Sprachen): network-first,
  // damit neue/aktualisierte Inhalte ohne SW-Release ankommen.
  if (/^\/(?:[a-z]{2}\/)?(?:blog|teams|promo-codes)(?:\/|$)/.test(reqPath)) {
    event.respondWith(
      fetch(event.request)
        .then((response) => {
          if (response.ok) {
